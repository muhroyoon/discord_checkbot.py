...
async def announce_last_month():
    ...
    for m in eligible_members:
        u=users.get(str(m.id))
        if u:
            ranking_list.append((m,u.get("monthly",{}).get(last_month,0)))
...
async def update_stats_channels(bot):
    ...
    today_count=len([u for u in users.values() if u["last_attendance"]==datetime.now(KST).strftime("%Y-%m-%d")])
