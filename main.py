...
async def attendance_panel(interaction: discord.Interaction):
    ...
    await update_stats_channels(bot)  # 즉시 갱신
...
@tree.command(name="출석랭킹", description="이번 달 출석 랭킹 보기")
async def ranking(interaction: discord.Interaction):
    ...
    for m in eligible_members:
        u = data.get(str(m.id))
        if u:
            ranking_list.append((m.display_name,u.get("monthly",{}).get(month,0)))
    ranking_list.sort(key=lambda x:x[1],reverse=True)
    desc=""
    for i,(name,count) in enumerate(ranking_list):
        desc+=f"{i+1}위 {name} - {count}일\n"
    if not desc: desc="출석 데이터가 없습니다."
    embed = discord.Embed(title=f"🏆 {month} 출석 랭킹", description=desc, color=0xffcc00)
    await interaction.response.send_message(embed=embed)
...
async def announce_last_month():
    ...
    for m in eligible_members:
        u=data.get(str(m.id))
        if u: ranking_list.append((m,u.get("monthly",{}).get(last_month,0)))
    ranking_list.sort(key=lambda x:x[1],reverse=True)
    desc=""
    for i,(m,count) in enumerate(ranking_list):
        desc+=f"{i+1}위 {m.display_name} - {count}일\n"
    embed=discord.Embed(title=f"🏆 {last_month} 출석 랭킹", description=desc if desc else "출석 데이터가 없습니다.", color=0xff6600)
...
async def update_stats_channels(bot):
    ...
    if not today_channel or not total_channel: return
    ...
    try:
        if today_channel.name!=new_today_name: await today_channel.edit(name=new_today_name)
    except: pass
    try:
        if total_channel.name!=new_total_name: await total_channel.edit(name=new_total_name)
    except: pass
